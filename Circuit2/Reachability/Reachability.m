...
tf_max=150;  %Maximum final time for reachability test 
nt_solve=1000; 
deltat = (t_f-t_int)/nt_solve;
...
%Reachability tolerence
tol=10^-2;


%Initial guess for NCG
u_= 10;
...
test_tol=cost_f(P_main,P);
test_tol_1=test_tol;
test_optimal=test_tol_1;



%functions to store the cost function value at each final time
J_final=[test_tol_1];
Tf=[t_f];
...
fprintf('\n The current time is: %g with optimal value=%g',t_f,test_tol);
...
nt_solve=round((t_f-t_int)/deltat); %time disrectization of time windows 
...
filename1 = sprintf('reach_Pd_mu=%d.jpg',mu);
filename2 = sprintf('reach_u_mu=%d.jpg',mu);

filename3 = sprintf('reach_J_mu=%d.jpg',mu);

T=linspace(t_int,Tf(J_final==min(J_final)),round((Tf(J_final==min(J_final))-t_int)/deltat));
