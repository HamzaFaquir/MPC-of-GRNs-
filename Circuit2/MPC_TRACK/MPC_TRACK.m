...
%%%%%%% Code for the in silico tracking of several target  Ciurcuit 2 %%%%%%%%%%%%%%%%%
...
N_max=30; %N_max*T_sample the number of sample times to run mpc
N_ch=1; %control horizon
...
number_targets=4; %number of tracking targets 
P_track=cell(number_targets,1);
track_time_point=[0 100 200 300]; %time points to change the target size=number_target
indx=2;
track_step=50; %if option 1 to autmatically generate normal distribution for mu=mu+track_step
...
P=P/trapz(x{1},P);
    ...
    mu=100;
    PDF = 1*normpdf(x{1}, mu, 20);%
    params.opt.P_target = (PDF/trapz(x{1},PDF))'; % normalise it, i.e. sum equals to one.
    P_track{1}=params.opt.P_target;%Save initial target 
    for k=2:number_targets    
    mu=min(n_x,mu+track_step);
    PDF = 1*normpdf(x{1}, mu, 18);%
    P_track{k} = (PDF/trapz(x{1},PDF))'; % normalise it, i.e. sum equals to one.
    end 
...
cost_function(1)=max(abs(PX_sol{1}-params.opt.P_target));
...
if j~= m*N_max
    ...
    %update target at the defined tracking time points
    if j*deltat==track_time_point(indx)
        params.opt.P_target=P_track{indx};
        indx=min(indx+1,number_targets);
    end 
...
PX_sol{j} = PX;
P_k=PX;
cost_function(j)=max(abs(PX_sol{j}-params.opt.P_target));
...
%%%%Plot solution current time and animation    
 if mod(j,m)==0 || j==1
    ...
    hold off
     ...
     hold on 
     plot(x{1},params.opt.P_target,'--','LineWidth',1.5)
     xlabel('Protein 1')
     ylabel('Probability')
     title(sprintf('Time: %0.2f a.u, mu: %f', j*deltat,mu),...
    'Interpreter','Latex');
